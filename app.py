...
FIB_COLORS = {
    0.0:   "#FF4444", 0.236: "#FF8C00", 0.382: "#FFD700",
    0.500: "#00DD88", 0.618: "#2196F3", 0.786: "#9C27B0",
    1.0:   "#FF4444", 1.272: "#FF69B4", 1.414: "#00BCD4",
    1.618: "#4CAF50", 2.000: "#FF5722", 2.618: "#673AB7",
}

FIB_LABELS = {
    0.0:   "0.000  起點",   0.236: "0.236",
    0.382: "0.382  ★",     0.500: "0.500  中線",
    0.618: "0.618  ★",     0.786: "0.786",
    1.0:   "1.000  終點",   1.272: "1.272  延伸",
    1.414: "1.414  延伸",   1.618: "1.618  延伸★",
    2.000: "2.000  延伸",   2.618: "2.618  延伸",
}

INTERVAL_PERIODS = {
    "1m":  ["1d", "5d"],         "5m":  ["1d", "5d", "1mo"],
    "15m": ["5d", "1mo"],        "30m": ["1mo", "3mo"],
    "1h":  ["1mo", "3mo", "6mo"],"1d":  ["3mo", "6mo", "1y", "2y", "5y"],
    "1wk": ["6mo", "1y", "2y", "5y"], "1mo": ["1y", "2y", "5y"],
}
PERIOD_LABELS = {
...
def dominant_swing(df, window=10):
    ...
    if not hi_idx: hi_idx = [int(df["High"].values.argmax())]
    if not lo_idx: lo_idx = [int(df["Low"].values.argmin())]
    cutoff = max(0, len(df) - int(len(df) * 0.40))
    def best_high(idxs):
        r = [i for i in idxs if i >= cutoff] or idxs
        return max(r, key=lambda i: df["High"].iloc[i])
    def best_low(idxs):
        r = [i for i in idxs if i >= cutoff] or idxs
        return min(r, key=lambda i: df["Low"].iloc[i])
    h_i, l_i = best_high(hi_idx), best_low(lo_idx)
...
def fib_extension(high, low, trend="up"):
    diff = high - low
    if trend == "up":
        return {lvl: high + diff * (lvl - 1.0) for lvl in FIB_EXT}
    return {lvl: low - diff * (lvl - 1.0) for lvl in FIB_EXT}

def current_fib_position(price, levels):
    sorted_lvl = sorted(levels.items(), key=lambda x: x[1])
    below = above = None
    for lvl, p in sorted_lvl:
        if p <= price: below = (lvl, p)
        elif above is None: above = (lvl, p)
    if below and above:
        span = above[1] - below[1]
        return below[0], below[1], above[0], above[1], (price-below[1])/span if span else 0.5
    if below: return below[0], below[1], None, None, 1.0
    return None, None, above[0], above[1], 0.0

def trend_from_swings(hi_idx, lo_idx):
    return "up" if lo_idx > hi_idx else "down"

def bull_bear_signal(price, ret_levels, trend):
    r382, r500, r618 = ret_levels[0.382], ret_levels[0.500], ret_levels[0.618]
    h, l = ret_levels[0.0], ret_levels[1.0]
    score, tags = 50, []
    if trend == "up": score += 10; tags.append("上升趨勢")
    else:             score -= 10; tags.append("下降趨勢")
    if price > r382:  score += 15; tags.append("站上0.382")
    elif price < r618:score -= 15; tags.append("跌破0.618")
    if price > r500:  score += 10; tags.append("站上0.5中線")
    else:             score -= 5
    if price > h*0.99:score += 15; tags.append("逼近高點")
    if price < l*1.01:score -= 15; tags.append("逼近低點")
...
def tomorrow_targets(price, ret_levels, ext_levels, trend):
    all_lvl = sorted({**ret_levels, **ext_levels}.items(), key=lambda x: x[1])
    r1 = next(((lv,p) for lv,p in all_lvl if p > price), None)
    s1 = next(((lv,p) for lv,p in reversed(all_lvl) if p < price), None)
    if r1 and s1:
        bias = "偏多，關注壓力" if trend=="up" else "偏空，關注支撐"
        desc = (f"趨勢{bias}｜上方目標 {r1[1]:,.0f}（{FIB_LABELS.get(r1[0],f'{r1[0]:.3f}')}）｜"
                f"下方支撐 {s1[1]:,.0f}（{FIB_LABELS.get(s1[0],f'{s1[0]:.3f}')}）")
        return {"up_target":r1[1],"dn_target":s1[1],"pivot":price,"description":desc}
    elif r1:
        return {"up_target":r1[1],"dn_target":price*0.99,"pivot":price,
                "description":f"上方目標 {r1[1]:,.0f}（{FIB_LABELS.get(r1[0],'')}）"}
    elif s1:
        return {"up_target":price*1.01,"dn_target":s1[1],"pivot":price,
                "description":f"下方支撐 {s1[1]:,.0f}（{FIB_LABELS.get(s1[0],'')}）"}
    return {"up_target":price*1.01,"dn_target":price*0.99,"pivot":price,"description":"無明確費氏目標"}

def build_chart(df, name, ret_levels, ext_levels, swing_hi, shi, swing_lo, slo, trend, show_ext, tmr):
    ...
    fig.add_trace(go.Candlestick(
        ...
        increasing_fillcolor="#26a69a", decreasing_fillcolor="#ef5350", line_width=1,
    ), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(x=[x_axis[shi]], y=[swing_hi], mode="markers+text",
        marker=dict(symbol="triangle-down", size=14, color="#ef5350"),
        text=["▼波段高"], textposition="top center",
        textfont=dict(size=11, color="#ef5350"), showlegend=False), row=1, col=1)
    fig.add_trace(go.Scatter(x=[x_axis[slo]], y=[swing_lo], mode="markers+text",
        marker=dict(symbol="triangle-up", size=14, color="#26a69a"),
        text=["▲波段低"], textposition="bottom center",
        textfont=dict(size=11, color="#26a69a"), showlegend=False), row=1, col=1)
    ...
    fig.add_trace(go.Bar(x=x_axis, y=df["Volume"].tolist(),
                         marker_color=vol_colors, opacity=0.8, name="成交量"), row=2, col=1)

    diff = swing_hi - swing_lo if swing_hi != swing_lo else 1.0
    fib_osc = [max(0.0, min(1.0, (swing_hi-float(p))/diff)) for p in df["Close"]]
    osc_colors = ["#ef5350" if v>0.618 else ("#ffd700" if v>0.382 else "#26a69a") for v in fib_osc]
    fig.add_trace(go.Bar(x=x_axis, y=fib_osc, marker_color=osc_colors,
                         opacity=0.9, name="費氏位階"), row=3, col=1)
    ...
    fig.update_layout(
        ...
        title=dict(text=(f"<b>{name}</b>　｜　費氏數列分析　｜　現價 <b>{current:,.0f}</b>"
...
def card(title, value, sub="", css_class=""):
    return (f'<div class="card {css_class}"><h4>{title}</h4>'
            f'<div class="val">{value}</div><div class="sub">{sub}</div></div>')

def fib_table_html(levels, current_price):
    rows = ""
    for lvl, price in sorted(levels.items(), key=lambda x: x[1], reverse=True):
        diff  = current_price - price
        pct   = diff / price * 100
        arrow = "▲" if diff >= 0 else "▼"
        color = "#26a69a" if diff >= 0 else "#ef5350"
        hl    = 'class="fib-highlight"' if lvl in (0.382, 0.618) else ""
        rows += (f'<tr {hl}><td>{FIB_LABELS.get(lvl,f"{lvl:.3f}")}</td>'
                 f'<td style="color:{FIB_COLORS.get(lvl,"#888")}"><b>{price:,.0f}</b></td>'
                 f'<td style="color:{color}">{arrow} {abs(diff):,.0f} ({abs(pct):.2f}%)</td></tr>')
...
def sidebar():
    ...
    period_label = st.sidebar.selectbox("資料期間", valid_period_labels,
                                         index=min(2, len(valid_period_labels)-1))
    period = [k for k,v in PERIOD_LABELS.items() if v == period_label][0]
    st.sidebar.markdown("---")
    swing_window = st.sidebar.slider("波段偵測靈敏度（K棒數）", 3, 30, 10, 1,
    ...
    if auto_refresh: st.sidebar.info("每3分鐘自動刷新資料")
    ...
    return {"ticker":ticker, "ticker_label":ticker_label.split()[0],
...
def main():
    ...
    if df.empty:
        st.error("無法取得資料，請確認商品代碼或稍後再試。"); st.stop()
    ...
    signal  = bull_bear_signal(current, ret, trend)
    tmr     = tomorrow_targets(current, ret, ext, trend)
    lb, lb_p, ab, ab_p, pct_in = current_fib_position(current, ret)

    col1,col2,col3,col4,col5 = st.columns(5)
    with col1: st.markdown(card("現價", f"{current:,.0f}",
        f"{'▲' if change>=0 else '▼'} {change:+,.0f} ({change_pct:+.2f}%)",
        "card-green" if change>=0 else "card-red"), unsafe_allow_html=True)
    with col2: st.markdown(card("波段高點", f"{swing_hi:,.0f}", "起算點（Fib 0.0）","card-red"),
        unsafe_allow_html=True)
    with col3: st.markdown(card("波段低點", f"{swing_lo:,.0f}", "起算點（Fib 1.0）","card-green"),
        unsafe_allow_html=True)
    with col4:
        sig_css = "card-green" if signal["css"]=="bull" else ("card-red" if signal["css"]=="bear" else "card-gold")
    ...
    with col5:
        ...
        st.markdown(card("趨勢方向", "↑ 多頭" if trend=="up" else "↓ 空頭",
            f"波段漲幅 {abs(swing_hi-swing_lo):,.0f} ({abs(swing_hi-swing_lo)/swing_lo*100:.1f}%)", t_css),
            unsafe_allow_html=True)
    st.markdown("")

    tab1, tab2, tab3, tab4 = st.tabs(["📈 K線 + 費氏分析","🎯 明日目標價","📋 費氏層級表","📊 多時框概覽"])

    with tab1:
        fig = build_chart(df, cfg["ticker_label"], ret, ext,
                          swing_hi, sh_idx, swing_lo, sl_idx, trend, cfg["show_ext"], tmr)
        st.plotly_chart(fig, use_container_width=True)
        if lb and ab:
            pos_txt = (f"現價 **{current:,.0f}** 位於費氏 **{lb:.3f}** ({lb_p:,.0f}) ↔"
                       f" **{ab:.3f}** ({ab_p:,.0f}) 之間，位置 {pct_in*100:.1f}%")
        elif lb:
            pos_txt = f"現價 **{current:,.0f}** 已站上所有費氏回檔位，費氏 {lb:.3f} 為支撐"
        else:
            pos_txt = f"現價 **{current:,.0f}** 低於所有費氏回檔位，費氏 {ab:.3f} 為壓力"
        st.info(f"📍 **費氏位置**: {pos_txt}")
        sig_html = (f'<span class="{signal["css"]}" style="font-size:18px;font-weight:bold">'
                    f'{signal["label"]}</span>　'
                    f'<span style="color:#aaa;font-size:14px">{"　".join(signal["tags"])}</span>')
        st.markdown(f"🚦 **多空判斷**: {sig_html}", unsafe_allow_html=True)

    with tab2:
        now_tw  = datetime.now(TW_TZ)
        nxt = now_tw + timedelta(days=1)
        while nxt.weekday() >= 5: nxt += timedelta(days=1)
        st.markdown(f"### 🎯 明日目標價　`{nxt.strftime('%Y-%m-%d (%A)')}`")
        st.markdown(f"> {tmr['description']}")
        st.markdown("")
        c1,c2,c3 = st.columns(3)
        with c1: st.markdown(card("⬆ 上方目標（壓力）", f"{tmr['up_target']:,.0f}",
            f"距現價 +{tmr['up_target']-current:,.0f} (+{(tmr['up_target']-current)/current*100:.2f}%)","card-red"),
            unsafe_allow_html=True)
        with c2: st.markdown(card("⏺ 今日收盤（樞軸）", f"{current:,.0f}",
            f"波段位置 {(swing_hi-current)/(swing_hi-swing_lo)*100:.1f}% 回檔","card-gold"),
            unsafe_allow_html=True)
        with c3: st.markdown(card("⬇ 下方支撐", f"{tmr['dn_target']:,.0f}",
            f"距現價 {tmr['dn_target']-current:,.0f} ({(tmr['dn_target']-current)/current*100:.2f}%)","card-green"),
            unsafe_allow_html=True)
        st.markdown("")
        st.markdown("#### 📌 關鍵費氏支撐壓力一覽")
        all_fib = {**ret, **(ext if cfg["show_ext"] else {})}
        ...
        fig2 = go.Figure(go.Bar(
            x=labels_f, y=[abs(p-current) for p in prices_f],
            marker_color=["#ef5350" if p>current else "#26a69a" for p in prices_f],
            text=[f"{p:,.0f}" for p in prices_f], textposition="outside"))
        fig2.update_layout(template="plotly_dark", paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
            height=300, margin=dict(l=10,r=10,t=30,b=10),
            title="各費氏層級與現價的距離（綠=支撐 / 紅=壓力）", showlegend=False,
        ...
        disp = hist[["Close","High","Low","Volume","費氏回檔%","費氏位階"]].copy()
        if hasattr(disp.index, 'strftime'): disp.index = disp.index.strftime("%Y-%m-%d %H:%M")
        disp.columns = ["收盤","最高","最低","成交量","費氏回檔%","費氏位階"]
        st.dataframe(disp.style.background_gradient(subset=["費氏回檔%"], cmap="RdYlGn_r"),
                     use_container_width=True)

    with tab3:
        st.markdown("### 📋 費氏回檔層級詳表")
        st.markdown(fib_table_html(ret, current), unsafe_allow_html=True)
        if cfg["show_ext"]:
            st.markdown("### 📋 費氏延伸層級詳表")
            st.markdown(fib_table_html(ext, current), unsafe_allow_html=True)
        st.markdown("#### 費氏數列說明")
        st.markdown("""
| 層級 | 意義 |
|------|------|
...
| **1.618** | 延伸目標，波段完成位 |
    ...
    with tab4:
        ...
        mtf_cfg = {"日線 (3個月)":("1d","3mo"),"日線 (1年)":("1d","1y"),
                   "週線 (2年)":("1wk","2y"),"月線 (5年)":("1mo","5y")}
        results, scores, labels_mtf = [], [], []
        prog = st.progress(0)
        for idx,(lbl,(iv,pr)) in enumerate(mtf_cfg.items()):
            ...
            results.append({"時間框架":lbl,"現價":f"{cp_m:,.0f}","波段高":f"{sh_m:,.0f}",
                            "波段低":f"{sl_m:,.0f}","費氏回檔%":f"{ret_pct:.1f}%",
                            "趨勢":"↑多" if tr_m=="up" else "↓空","多空訊號":sig_m["label"],
                            "明日目標↑":f"{tmr_m['up_target']:,.0f}","明日支撐↓":f"{tmr_m['dn_target']:,.0f}"})
            labels_mtf.append(lbl); scores.append(sig_m["score"])
        prog.empty()
        if results:
            st.dataframe(pd.DataFrame(results).set_index("時間框架"), use_container_width=True)
            fig3 = go.Figure(go.Bar(x=labels_mtf, y=scores,
                marker_color=["#26a69a" if s>=60 else ("#ffd700" if s>=45 else "#ef5350") for s in scores],
            ...
            fig3.update_layout(template="plotly_dark", paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
                height=300, margin=dict(l=10,r=10,t=30,b=10), title="多時框多空強度評分",
                showlegend=False, yaxis=dict(range=[0,110]))
            st.plotly_chart(fig3, use_container_width=True)

    st.markdown("---")
    st.markdown('<div style="text-align:center;color:#555;font-size:12px">'
                '資料來源：Yahoo Finance ｜ 費氏數列分析看板 ｜ 僅供參考，不構成投資建議</div>',
                unsafe_allow_html=True)
